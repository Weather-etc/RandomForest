...
(vector<vector<Field>> X, vector<int> y, vector<int> columns) {
    ...
    for (auto it : columns) {
        ...
        // TODO: Convert branches into computation : Highlight!

#pragma omp parallel num_threads(4)
        {
#pragma omp for
            for (int i = 0; i < y.size(); i++) {
                CountTimes[X[it][i].str_value].first += 1 - y[i];
                CountTimes[X[it][i].str_value].second += y[i];
                /*
                if (y[i] == 1)
        ...
        //float states[times_vec.size()];
        float *states = new float[times_vec.size()];
        // TODO: Parallel this
        ...
        // TODO: 1. Detach branches     2. Use matrix
        ...
        for (int i = 0; i < times_vec.size() - 1; i++) {
            ...
            total_R_class0 -= times_vec[i].second.first;
            total_R_class1 -= times_vec[i].second.second;
            states[i] = total_L_class0 * total_L_class1 / (total * total_state) +
                total_R_class0 * total_R_class1 / (total * (total - total_state));
